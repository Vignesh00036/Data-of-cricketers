...
# It is going to return the name and team_number for selected teams
...
# Appending the team teams in teams variable and skipping the under-19 and womens teams
		...
		for i in range(how_many_teams):
			...
			if team_to_insert=='All' or team_to_insert=='ALL' or team_to_insert=='79':
				...
				for j in range(int(starting_team[1]), len(team_numbers)):
					print(f'Scarapping {teams[j]} data')
					data_collector.navigator(driver, page_number, team_numbers[j], teams[j], matches)
					data_snowflake.importing_files(teams[j])
					data_aws_s3.upload(teams[j])
